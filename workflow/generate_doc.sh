...
# copies associated images, and builds ./docs/README.md.
...
# Extract header for docs (# <feature>)
extract_header() {
	local conf_file="$1"
...
module_image_markdown() {
	...
	for ext in png jpg jpeg gif svg; do
		...
		if [[ -f "$img" ]]; then
			cp "$img" "$DOC_ROOT/"
			echo "![${modname}](${modname}.${ext})"$'\n'
			return
		fi
	done
}

# Extracts metadata description and extended description
extract_metadata_rest() {
	local conf_file="$1"
	local description extend_desc
...
generate_module_docs() {
	...
	while IFS= read -r sh_file; do
		...
		{
			extract_header "$conf_file"
			module_image_markdown "$modname" "$(dirname "$sh_file")"
			extract_metadata_rest "$conf_file"
