...
_check_conf_stable() {
	local REQUIRED_CONF_FIELDS=(feature helpers description parent group contributor port)
	file="$1"
	local failed=0
	local failed_fields=()
	if [ ! -f "$file" ]; then
	...
	for field in "${REQUIRED_CONF_FIELDS[@]}"; do
		...
		# helpers field must contain at least one _about_...
		if [ "$field" = "helpers" ]; then
			if ! echo "$value" | grep -q '_about_'; then
				failed=1
				failed_fields+=(" $field (no helper listed; must have at least one _about_)")
			fi
		fi
	done
	...
	if [ "$failed" -eq 0 ]; then
		...
		echo -e "FAIL: $file missing or invalid fields:\n${failed_fields[*]}"
		return 1
	fi
}
